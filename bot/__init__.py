    ...
    if (pv := event.is_private) or event.is_group :
        if pv:
            try:
                ...
                await event.reply(f"אין חיבור לקבוצה הראשית\n\n@{Config.CHANNEL_USERNAME}\n\nלאחר ההתחברות יש ללחוץ על -  /start ")
                return
        ...
        if event.file :
            ...
            await event.reply(f"{Config.DOMAIN}/{id}")
            return
        ...
        if pv:
            await event.reply("שלח לי קובץ ואתן לך לינק הורדה ישירה :)")
